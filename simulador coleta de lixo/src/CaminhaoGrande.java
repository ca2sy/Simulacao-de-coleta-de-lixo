public class CaminhaoGrande {
    ...
    public int tempo_esperado; //quanto tempo o caminhao ja esperou

    public CaminhaoGrande(String id_caminhao_grande) {
        this.carga_atual = 0;
        this.tempo_esperado = 0;
    ...
    public void aguardarNaEstacao(int tempo) {
        tempo_esperado += tempo;
        System.out.println("Caminhão " + id_caminhao_grande + "  aguardou " + tempo + " minutos. Tempo total de espera: " + tempo_esperado);
        if(tempo_esperado >= tolerancia && carga_atual > 0){
            System.out.println("Tempo de espera excedido e caminhão " + id_caminhao_grande + "  possui carga, partindo para o aterro.");
            descargaNoAterro();
        } else if (tempo_esperado >= tolerancia && carga_atual == 0){
            System.out.println("Tempo de espera excedido, mas caminhão " + id_caminhao_grande + "  está vazio, aguardando um carregamento.");
        }
    }
